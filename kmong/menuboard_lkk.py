...
class MyWindow(QMainWindow, form_class):  # MyWindow Class로 생성 후, QMainWindow, UI 상속받기
    def __init__(self):
        ...
        self.click_counts = {}
        df0 = pd.read_csv('List_shrimp.csv', index_col=0).iloc[:,0]
        df1 = pd.read_csv('List_squid.csv', index_col=0).iloc[:,0]
        df2 = pd.read_csv('List_fish.csv', index_col=0).iloc[:,0]
        df3 = pd.read_csv('List_meat.csv', index_col=0).iloc[:,0]
        df4 = pd.read_csv('List_other.csv', index_col=0).iloc[:,0]
    ...
    def L1_table_editq(self):
        ...
        if selected_box[0].column() == 3:  # 수량 +
            ...
            self.L1_table.setItem(selected_box[0].row(), 2, QTableWidgetItem(str(int(self.df_cnt.iat[add_df_col, add_df_row]))))  # table에 cnt update
        ...
        if selected_box[0].column() == 4:  # 수량 -
            ...
            if self.df_cnt.iat[sub_df_col, sub_df_row] == 1:
                ...
                self.L1_table.setItem(selected_box[0].row(), 2, QTableWidgetItem(str(int(self.df_cnt.iat[sub_df_col, sub_df_row]))))  # table에 cnt update
    ...
    def L2_btnq(self):  # History저장
        ...
        if self.L2_text.toPlainText() == '':
            ...
            with open(os.getcwd() + '\Order_History.csv', 'w', newline="", encoding='cp949') as csvfile:
